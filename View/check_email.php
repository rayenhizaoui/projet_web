...
if(isset($_POST['reset_link'])){
        ...
        //$query = $pdo->prepare("SELECT login FROM users where login = ?");
        $query->execute([$login]);
        $row = $query->rowCount();
        ...
        if($row == 1){
            ...
            // Formulate the link
            $link = 'href="http://localhost/projects/tutorial/reset_password/reset_password.php?email='.$login.'&code='.$code.'"';
            
            $link2 = '<span style="width:100%;"><a style="padding:10px 100px;border-radius:30px;background:#a8edbc;" '.$link.' > Link </a></span>';
    
            //echo $code, $link; 
    
            $query_exist =  $pdo->prepare("SELECT * FROM reset where login = ?");
            $query_exist->execute([$login]);
            $from_reset = $query_exist->fetch();
    
            if(empty($from_reset)){
                // Save code and INSERT email in a database
                $query_insert = $pdo->prepare("INSERT INTO reset(login, code) VALUES (?, ?)");
                $query_insert->execute([$login, $code]);
            } else {
                // Already exist reseting attempt, switch to UPDATE the reset table instead
                $query_insert = $pdo->prepare("UPDATE reset SET code = ? WHERE login = ?");
                $query_insert->execute([$code, $login]);
            }
